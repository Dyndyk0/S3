...
public static class ValueMetadataEndpoints
{
    ...
    {
        // GET /valuemetadata
        app.MapGet("/valuemetadata", async ([AsParameters] ValueMetadataFilterDto filter, DbService db) => {
            IEnumerable<ValueMetadataDto> metadata = await db.GetValueMetadataAsync(filter);
            return Results.Ok(metadata);
        });

        // POST /valuemetadata
        app.MapPost("/valuemetadata", async (int keyMetadataId, string name, DbService db) => {
            await db.CreateValueMetadataAsync(keyMetadataId, name);
            return Results.Ok();
        });

        // PATCH /valuemetadata
        app.MapPatch("/valuemetadata", async (ValueMetadataDto dto, DbService db) => {
        ...
        app.MapDelete("/valuemetadata", async (int id, DbService db) => {
